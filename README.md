# Hi, I'm Elias 👋

📍 Philadelphia | 🔧 Full-Stack Builder | 🎬 VFX + AI

[![Swift](https://img.shields.io/badge/-Swift-FA7343?style=flat-square&logo=swift&logoColor=white)](https://swift.org)
[![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)](https://typescriptlang.org)
[![Go](https://img.shields.io/badge/-Go-00ADD8?style=flat-square&logo=go&logoColor=white)](https://go.dev)
[![React](https://img.shields.io/badge/-React-61DAFB?style=flat-square&logo=react&logoColor=black)](https://react.dev)
[![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)](https://nodejs.org)
[![Codex](https://img.shields.io/badge/-Codex-121212?style=flat-square&logo=openai&logoColor=white)](https://openai.com)
[![Claude](https://img.shields.io/badge/-Claude-000000?style=flat-square&logo=anthropic&logoColor=white)](https://www.anthropic.com)
[![CLI](https://img.shields.io/badge/-CLI-000000?style=flat-square&logo=gnu-bash&logoColor=white)](https://en.wikipedia.org/wiki/Command-line_interface)
[![macOS](https://img.shields.io/badge/-macOS-000000?style=flat-square&logo=apple&logoColor=white)](https://www.apple.com/macos)
[![Web](https://img.shields.io/badge/-Web-4285F4?style=flat-square&logo=google-chrome&logoColor=white)](https://developer.mozilla.org/en-US/docs/Web)

Founder of **[Reality Shifting Tech](https://realityshifting.tech)** — a dev agency building web apps, mobile apps, blockchain tools, AR filters, and AI-powered experiences for creators and brands.

- 140K on Instagram ([@elias.effects](https://instagram.com/elias.effects)) from VFX work.
- 135K on TikTok ([@elias.effects](https://www.tiktok.com/@elias.effects)) mainly from AR filters.

---

## Current Projects

- 🦀 **[clawtally](https://github.com/eliaseffects/clawtally)** — Read-only telemetry dashboard for OpenClaw with local-first usage insights and optional leaderboard sharing.
- 📄 **[llms-txt-generator](https://github.com/eliaseffects/llms-txt-generator)** — Turn docs into agent-ready discovery files (`llms.txt` + `agent.json`).
- 🐦 **[x-api-cli](https://github.com/eliaseffects/x-api-cli)** — Node.js CLI for posting and deleting tweets using the official X API v2.
- 🧠 **[stack-detector](https://github.com/eliaseffects/stack-detector)** — One-click website stack scanner.
- 🔲 **[qr-cli](https://github.com/eliaseffects/qr-cli)** — Blazingly fast QR code generator for your terminal. Built in Go.

---

## What I Build

| Category | Stack |
|----------|-------|
| **Web Apps** | Next.js, React, Node, Tailwind, Prisma |
| **Mobile** | Flutter, React Native, Swift |
| **Blockchain** | Solana, EVM, Web3.js |
| **AR Filters** | Effect House, Spark AR, Lens Studio |
| **AI Tools** | OpenAI, Anthropic, local models |
| **Video/VFX** | After Effects, Premiere, DaVinci |

---

## What I'm Doing

- **Building with AI-native workflows** - Using agent tooling to move from idea to shipped product fast
- **Scaling Reality Shifting Tech** - Delivering software, AR, and AI products for creators and brands
- **Rapid prototyping** - Shipping useful tools in days, not months
- **Creating in public** - Sharing VFX + dev progress on [Instagram](https://instagram.com/elias.effects)

---

## Latest Blog Posts

- [Build updates and threads on X](https://twitter.com/eliaseffects)
- [Project release notes on GitHub](https://github.com/eliaseffects?tab=repositories)
- [Reality Shifting Tech updates](https://realityshifting.tech)

---

## Connect

[![Website](https://img.shields.io/badge/-realityshifting.tech-000?style=flat-square&logo=vercel&logoColor=white)](https://realityshifting.tech)
[![Twitter](https://img.shields.io/badge/-@eliaseffects-1DA1F2?style=flat-square&logo=twitter&logoColor=white)](https://twitter.com/eliaseffects)
[![Instagram](https://img.shields.io/badge/-@elias.effects-E4405F?style=flat-square&logo=instagram&logoColor=white)](https://instagram.com/elias.effects)
[![LinkedIn](https://img.shields.io/badge/-eliasstevenson-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/eliasstevenson)

---

## Recognition

- 140K+ Instagram audience built through VFX content and creative tech experiments
- Founder of [Reality Shifting Tech](https://realityshifting.tech)
- Full-stack builder across web, mobile, blockchain, AI, and AR experiences

---

## Media

- [Instagram: @elias.effects](https://instagram.com/elias.effects)
- [LinkedIn: Elias Stevenson](https://linkedin.com/in/eliasstevenson)
- [GitHub: @eliaseffects](https://github.com/eliaseffects)

---

> "Ship beats perfect."
